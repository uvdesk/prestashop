					                ...
					                {if isset($attachments) && $attachments}
					                	<div class="attachments first-attach">
						                	{foreach $attachments as $attachment}
						                    	<a href="{$link->getAdminLink('AdminUvdeskTicketList')}&attach={$attachment->id}">
						                    		<i class="icon-download wk-attachment" data-attachment-id="{$attachment->id}" title="{$attachment->name}"></i>
								                </a>
						                    {/foreach}
					                   	</div>
					                {/if}
		            	...
		            	<div class="pos-relative">
					 	...
					 	{if isset($currentCustomer->data['0']) }
						  	<div class="btn btn-primary btn-sm selected_person closefilter" data-action="customer">
								<span>X</span>
						  		&nbsp;{$currentCustomer->data['0']->firstName} {$currentCustomer->data['0']->lastName}
							</div>
						{/if}
		          	</div>
		          	<div>
										...
										{foreach $customerTickets as $key => $tickets}
											<tr class="{if $key%2 == 0}even{else}odd{/if}" id="wk_ticket_row_{$tickets->id}">
												...
												<td>
													...
													<div class="wk-uvdesk-dropdown-menu open wk-agent-list" data-ticket-id="{$tickets->incrementId}"></div>
												</td>
												<td>
													<center>
