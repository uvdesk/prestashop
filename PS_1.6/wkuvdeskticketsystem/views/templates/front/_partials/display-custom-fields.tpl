            ...
            <div class="wk-custom-field-container">
                {foreach $customFieldValues as $fieldValues}
                    <div class="wk-custom-field-display">
                        <label>{$fieldValues.fieldName|escape:'htmlall':'UTF-8'}</label>
                        <div class="wk-field-block">
                            {if $fieldValues.fieldType == 'file' && isset($fieldValues.value->id)}
                                <a href="{if isset($backendController)}{$link->getAdminLink('AdminUvdeskTicketList')|escape:'htmlall':'UTF-8'}&attach={$fieldValues.value->id|escape:'htmlall':'UTF-8'}{else}{$link->getModuleLink('wkuvdeskticketsystem', 'viewticket', ['attach' => $fieldValues.value->id])|escape:'htmlall':'UTF-8'}{/if}">
                                    <i class="icon-download wk-attachment" data-attachment-id="{$fieldValues.value->id|escape:'htmlall':'UTF-8'}"></i>
                                </a>
                            {else}
                                {if is_array($fieldValues.visibleValues)}
                                    {foreach $fieldValues.visibleValues as $visiValues}
                                        {$visiValues|escape:'htmlall':'UTF-8'}<br>
                                    {/foreach}
                                {else}
                                    {$fieldValues.visibleValues|escape:'htmlall':'UTF-8'}
                                {/if}
                            {/if}
                        </div>
                    </div>
